...
export default function UpdateInsertRecord() {
const navigate=useNavigate();
const {setStuentData,setInsertRecod}=useContext(StudentContext)
...
// delete records
const updateInsertRecords=async(itm,item)=>{
setStuentData(item);
setInsertRecod(itm);
navigate("/updateInsertRecordForm");
  ...
  return (
    ...
    <TopBar />
    <h1>Click on Activity Which Wanted to Update</h1>
<div className="tt">
...
{demos.length===0? "NO  RCORD FOUND":<>
<h1>update Insert Record</h1>
<div className="th2">
...
return (
<div className="td3" onClick={()=>updateInsertRecords(itm,item)} key={itm._id}>
...
<div className="opt3"  style={{cursor:"pointer"}} ><MdSystemUpdateAlt id="icon"/></div>
</div>
)
})}
</div> 
